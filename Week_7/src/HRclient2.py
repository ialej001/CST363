...
def main():
    ...
    c.sendToAll("drop table if exists temp")

    c.sendToAll("create table tempd (dept int, dept_name char(20), mgr_id int, mgr_name char(20), mgr_salary double)")
    c.sendToAll("create table tempe (empid int, name char(20), dept char(20), salary double)")
    c.sendToAll("create table temp (empid int, emp_name char(20), emp_salary double, dept_name char(20), mgr_name "
                "char(20), mgr_salary double)")

    c.sendToAll("map select * from employee")
    c.sendToAll("shuffle insert into tempe values {}")

    c.sendToAll("map select d.dept, d.dept_name, d.mgr_id, e.name, e.salary from department d join employee e on "
                "d.mgr_id = e.empid")
    c.sendToAll("shuffle insert into tempd values {}")

    c.sendToAll("map select e.empid, e.name, e.salary, d.dept_name, d.mgr_name, d.mgr_salary from tempe e join tempd d "
                "on e.dept = d.dept where e.salary > d.mgr_salary")
    c.sendToAll("shuffle insert into temp values {}")

    print("Results")
    c.sendToAll("reduce select empid, emp_name, emp_salary, dept_name, mgr_name, mgr_salary from temp order by "
                "dept_name")
