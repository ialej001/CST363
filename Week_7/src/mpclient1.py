...
def createTable():
    ...
    with open('students.data', 'w') as file:
        for id in ids:
            major = randMajor()
            name = randName()
            gpa = str(round(randGPA(), 2))
            line = str(id) + ', "' + name + '", "' + major + '", ' + gpa + "\n"
            file.write(line)
    return ids[0]
...
def randID():
    ...
    random.seed()
    while len(IDs) < 100:
        id_to_be_added = random.randint(100, 999)
        if id_to_be_added not in IDs:
            IDs.append(id_to_be_added)
    return IDs
...
def randName():
    random.seed()
    firsts = ['Liam', 'Noah', 'William', 'James', 'Logan', 'Benjamin', 'Mason', 'Elijah', 'Oliver', 'Jacob', 'Emma',
...
def main():
    ...
    # create student data table and save first id that is created
    first_student = createTable()

    # send to worker nodes
    c = Coordinator()
    c.sendToAll("drop table if exists students")
    c.sendToAll("create table students (id int primary key, name char(20), major char(10), gpa double)")
    c.loadTable("students", "students.data")

    # verify
    print("Info on student id that exists")
    c.sendToAll("select * from students where id=%s" % first_student)
    print("\nInfo on student id that does not exist")
    c.sendToAll("select * from students where id=99")
    print("\nAll majors in Biology")
    c.sendToAll("select * from students where major= 'Biology'")
    print("\nGPA >= 3.2")
    c.sendToAll("select * from students where gpa >= 3.2")
